"""Plan 014 immutable admission, diagnostics and fail-closed stage packaging."""
import os
...
STAGES=['prepare','recover','diagnose','condition','basins','pilot','package']
SOURCES=['scripts/basketball_shared_'+n+'_v8.py' for n in ['workflow','diagnose','report','recovery']]


def deadline(p,stage):
    return min(p['investigation_started_unix']+60*p['v8']['deadlines_minutes'][stage],p['investigation_started_unix']+(14400 if stage=='package' else 13200))
...
def source_hashes(config,stage):
    names={'prepare':['diagnose','recovery'],'recover':['recovery'],'diagnose':['diagnose','verify','recovery'],
           'condition':['conditioning','profiles','solver','regressions','scalar_search'],
           'basins':['scalar_search','regressions','solver'],'pilot':['profiles','scalar_search','solver'],
           'package':['report']}[stage]
    files=['scripts/basketball_shared_workflow_v8.py',*['scripts/basketball_shared_'+n+'_v8.py' for n in names]]
...
def predecessor(a):
    ...
    if prior['schema']!='basketball-shared-timing-stage/v8' or prior['config_sha256']!=sha256(a.config):raise ValueError('predecessor schema/config mismatch')
    verify_hashes(prior['source_sha256']);verify_hashes(prior['artifacts_sha256'])
    allowed={'recover':['prepare'],'diagnose':['recover'],'condition':['diagnose'],'basins':['condition'],'pilot':['basins'],'package':STAGES[:-1]}
...
def finish(a,p,result,sources,started):
    ...
    result.update(schema='basketball-shared-timing-stage/v8',stage=a.stage,config_sha256=sha256(a.config),source_sha256=sources,
...
def stage(a):
    ...
    write(a.output/'frozen.json',dict(config=p,source_sha256=sources,frozen_unix=time.time(),namespace='v8-'+a.stage))
    try:
        check(p,a.stage)
        if a.stage=='package':
            from basketball_shared_report_v8 import package
            result=package(a.output,prior,a.predecessor,p)
        elif prior is not None and prior['status']!='passed' and not (a.stage=='basins' and prior.get('terminal_kind')=='scientific_rejection'):
            ...
            import importlib
            module={'prepare':'diagnose','recover':'recovery','diagnose':'diagnose','condition':'conditioning','basins':'scalar_search','pilot':'profiles'}[a.stage]
            implementation=importlib.import_module('basketball_shared_'+module+'_v8')
    ...
    except (ValueError,ArithmeticError,AssertionError,KeyError,ImportError) as e:
