"""Plan 010 evaluator investigation with cross-version admission and hard deadlines."""
import os
...
from basketball_shared_solver_v4 import solve,provenance

IMPLEMENTATION=['scripts/basketball_shared_'+name+'_v4.py' for name in ['solver','workflow','profiles','diagnose']]
...
def prepare(p,output):
            ...
            for length in [100,50,25]:
                for noise in [0.,.25,.5]:
                    for delta in p['injections']:
                        ...
                        recipes.append(dict(case_id=i,**recipe,path=str(f),sha256=sha256(f),independent_qualification='requires v4 reassessment'));i+=1
    ...
    manifest=dict(predecessor_schema=prior['schema'],predecessor_config_sha256=oldsha,
                  new_config_sha256=sha256('configs/basketball-rev2/timing-shared-v4.json'),
...
from basketball_shared_diagnose_v4 import diagnose
...
def stage(a):
    ...
    try:
        if a.stage=='prepare':result=prepare(p,a.output)
            ...
            from basketball_shared_profiles_v4 import safeguard
    ...
    result.update(schema='basketball-shared-timing-stage/v4',stage=a.stage,config_sha256=sha256(a.config),source_sha256=sources,
...
def main(a):
    ...
    try:return child.wait(timeout=max(.001,p['investigation_started_unix']+limit-time.time()))
        ...
        write(a.output/'result.json',dict(schema='basketball-shared-timing-stage/v4',stage=a.stage,status='blocked',terminal_kind='budget_exhaustion',
