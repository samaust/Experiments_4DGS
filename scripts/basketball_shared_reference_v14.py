...
class Reference:
    def __init__(self,case,setup,check=lambda:None):self.case=case;self.setup=setup;self.check=check
    def evaluate(self,state,audit):
        owner=audit
        if hasattr(owner,'observe'):
            actual=dict(state)
            audit=lambda kind,fn:owner(kind,lambda:owner.observe(kind,actual,fn))
        c=self.case;p=c['problem'];x=arr(state['x_hex']);U=list(arr(c['knots_hex']));nf=len(p['free']);nc=len(U)-4;m=len(x);C=x[nf:].reshape(nc,3)
        ...
        def values():
            ...
            gp=np.array(gps);return np.array(gs),gp,np.array(gpps),arr(state['vg_hex'])*gp
        ...
        def bounded():
            H=out['Hz'].copy()
            for v,gpp,J in zip(arr(state['vg_hex']),out['gpp'],out['Jz']):H+=(v*gpp)*np.outer(J,J)
