...
def safeguard(p,output):
    ...
    sources={f:sha256(f) for f in ['scripts/basketball_shared_profiles_v4.py','scripts/basketball_shared_solver_v4.py']}
    ...
    return dict(status='blocked',terminal_kind='numerical_failure',blockers=['observable escape remains unresolved after finite amplitude probes'],exact_retest=row,independent_controls_attempted=0,targeted_controls_attempted=0)
