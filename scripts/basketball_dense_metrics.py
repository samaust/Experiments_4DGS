"""Plan 027 metrics in separate recipe directories using the frozen metric worker."""
import argparse
import json
from pathlib import Path

from basketball_study import CURVE, ROOT, digest, supervise, verify_files, write_new
...
def main():
    ...
    runtime = json.loads((ROOT / 'docs/experiments/basketball-dense-temporal/metric-runtime.json').read_text())
    records = []
    for arm in (a.arm,):
        for seed in range(3):
    ...
    inputs = ARTIFACTS / f'metric-inputs-{a.arm}-{a.iteration:06d}.json'
    write_new(inputs, dict(runs=[dict(method=r['method'], seed=r['seed'], render=r['render']) for r in records]))
    output = ARTIFACTS / f'metrics/{a.arm}/{a.iteration:06d}'
    command = ['docker', 'run', '--rm', '--name', f'plan027-metrics-{a.arm}-{a.iteration}', '--network', 'none',
    ...
    segment = supervise(command, 'evaluation', ARTIFACTS / f'segments/metrics-{a.arm}-{a.iteration}')
