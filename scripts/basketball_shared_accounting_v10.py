...
from basketball_shared_storage_v10 import Observed
...
class Ledger:
    ...
    def __init__(self,problem_key,scale,check=lambda:None,max_states=200,max_iterations=200,journal=None):
    ...
    def request(self,q,kind,y=None,scope='destination'):
        ...
        if key not in self.states:
            ...
            if self.journal:self.journal.append("state",identity=identity,scope=scope,canonical_hex=bytes_of(q).hex(),physical_hex=bytes_of(x).hex())
    ...
    def enter(self,state,kind,callback):
        ...
        if self.journal:self.journal.state=state.identity
...
class NumericalEntries:
    ...
    def __init__(self,p):
        self.journal=getattr(p,"journal",None)
    ...
    def observe(self,kind,x):
        if self.journal:self.journal.append('numerical_entry',index=len(self.observed),state=self.journal.state,kind=kind,physical_hex=bytes_of(x).hex())
        self.observed.append(Observed(dict(kind=kind,physical_hex=bytes_of(x).hex(),status='entered'),self.journal,len(self.observed)))
...
class CanonicalAdapter:
    def __init__(self,p,namespace='test',provenance=None):
        ...
        self.ledger=Ledger(self.problem_key,self.scale,p.check,journal=getattr(p,"journal",None));self.numerical=NumericalEntries(p)
        ...
        if getattr(p,"journal",None):p.journal.append("problem",problem_key=self.problem_key,problem=self.problem_data,scale_hex=bytes_of(self.scale).hex(),transform_hex=bytes_of(self.P).hex(),origin_hex=bytes_of(self.origin).hex())
    ...
    def set_transform(self,cold,conditioned=False,legacy=False):
        ...
        if getattr(self.p,"journal",None):self.p.journal.append("transform",transform_hex=bytes_of(self.P).hex(),origin_hex=bytes_of(self.origin).hex())
