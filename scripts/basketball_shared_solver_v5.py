"""Bounded-depth v5 independent evaluator; immutable v2 objective and state."""
import inspect
import time
import warnings
import numpy as np
...
def provenance():
    return dict(method='trust-constr', policy=POLICY, scipy=scipy.__version__,
        ...
        transformation='s/hypot(1,s)', transformation_scale=1., cache_version='v5', objective_hessian='2 J.T J sparse Gauss-Newton')
...
class ConstrainedProblem:
    ...
    def hess(self,q):
        started=time.monotonic();_,J=self.evaluate(q);result=2*(J.T@J)
        self.derivative_seconds+=time.monotonic()-started;self.problem.check();return result
