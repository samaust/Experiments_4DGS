...
def ready():
 ...
 assert not any(n.module and 'v14' in n.module for n in ast.walk(ast.parse(Path('scripts/basketball_shared_reference_v14.py').read_text())) if isinstance(n,ast.ImportFrom))
...
def worker():
 started=read(ROOT/'started.json');worker_guard(started,os.environ.get('V14_TOKEN'),os.getppid(),ROOT/'worker-consumed.json')
 ...
 import numpy as np
 import basketball_acceleration_decimal_v14 as decimal_candidate
 import basketball_shared_components_v14 as candidate
 import basketball_shared_accounting_v14 as accounting
 import basketball_shared_solver_v14 as solver
 import basketball_shared_reference_v14 as reference
