"""Plan 009 evaluator investigation with cross-version admission and hard deadlines."""
import os
...
from basketball_shared_solver_v3 import solve,provenance,objective_agreement

IMPLEMENTATION=['scripts/basketball_shared_solver_v3.py','scripts/basketball_shared_workflow_v3.py']


def bounded(p, package=False, early=False):
    limit=14400 if package else min(13200,5400 if early else 13200)
...
def prepare(p,output):
            ...
            for length in [100,50,25]:
                for noise in [0.,.25,.5]:
                    for delta in p['injections']:
                        ...
                        recipes.append(dict(case_id=i,**recipe,path=str(f),sha256=sha256(f),independent_qualification='requires v3 reassessment'));i+=1
    ...
    manifest=dict(predecessor_schema=prior['schema'],predecessor_config_sha256=oldsha,
                  new_config_sha256=sha256('configs/basketball-rev2/timing-shared-v3.json'),
...
def diagnostic_job(payload):
    gid,lag,weight,deadline=payload
    def check():
        if time.time()>=deadline:raise TimeoutError('90-minute diagnosis/retest deadline')
    groups,cameras,_,window=synthetic('direction_changes',0.,-.1,100,groups=12)
    rows={}
    for policy in ['unit','scaled','dense']:
        problem=SplineProblem([groups[gid]],cameras,{1:0.,2:float(lag),3:0.},window,10,weight,(1,2),check)
        rows[policy]=solve(problem,diagnostic_policy=policy,diagnostics=True)
    pairs=[dict(a=a,b=b,agrees=objective_agreement(rows[a]['objective'],rows[b]['objective']),
                absolute_difference=abs(rows[a]['objective']-rows[b]['objective']))
           for a,b in [('unit','scaled'),('scaled','dense'),('unit','dense')]]
    return dict(group_id=gid,lag=lag,weight=weight,solvers=rows,comparisons=pairs)


def diagnose(p,output):
    jobs=[(g,l,w,p['investigation_started_unix']+5400) for w in [1.,0.]
          for g in p['diagnostic_groups'] for l in p['diagnostic_lags']]
    comparisons=[]
    with ProcessPoolExecutor(max_workers=8,mp_context=multiprocessing.get_context('spawn')) as pool:
        for i,row in enumerate(pool.map(diagnostic_job,jobs)):
            bounded(p,early=True);write(output/f'comparison{i:03d}.json',row)
            comparisons.append({k:v for k,v in row.items() if k!='solvers'})
            print('diagnostic',i,row['group_id'],row['lag'],row['weight'],flush=True)
    write(output/'solver-comparisons.json',dict(comparisons=comparisons,provenance=provenance(),
        dense_reference_is_local=True,agreement_is_not_identifiability=True))
    return dict(status='passed',blockers=[],terminal_kind=None,diagnostic_problems=len(jobs))


def stage(a):
    p=read(a.config);bounded(p,a.stage=='package',a.stage=='diagnose');started=time.monotonic()
    ...
    if a.stage!='prepare':
        ...
        allowed={'diagnose':{'prepare'},'safeguard':{'diagnose'},'benchmark':{'safeguard'},'fit':{'benchmark'},'assess':{'fit'},'select':{'assess'},'package':{'diagnose','safeguard','benchmark','fit','assess','select'}}
    ...
    try:
        if a.stage=='prepare':result=prepare(p,a.output)
            ...
            from basketball_shared_profiles_v3 import safeguard
    ...
    result.update(schema='basketball-shared-timing-stage/v3',stage=a.stage,config_sha256=sha256(a.config),source_sha256=sources,
...
def main(a):
    if a.output.exists():raise FileExistsError('fresh output required')
    p=read(a.config);bounded(p,a.stage=='package',a.stage in ['diagnose'])
    limit=14400 if a.stage=='package' else 5400 if a.stage=='diagnose' else 13200
    ...
    try:return child.wait(timeout=max(.001,p['investigation_started_unix']+limit-time.time()))
        ...
        write(a.output/'result.json',dict(schema='basketball-shared-timing-stage/v3',stage=a.stage,status='blocked',terminal_kind='budget_exhaustion',
...
if __name__=='__main__':
    ...
    parser.add_argument('stage',choices=['prepare','diagnose','safeguard','benchmark','fit','assess','select','package'])
