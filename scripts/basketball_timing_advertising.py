"""Bounded fitting-only native tracking with appearance discontinuity diagnostics."""
import argparse
...
from basketball_timing_recovery import fit
...
def run(a):
    ...
    fitted=read(a.output/'fit.json'); edges=prior_graph['edges']+[e for e in fitted['edges'] if e['passed']]
    ...
    write(a.output/'result.json',dict(**result,edges=edges,calibration_sha256=CALIBRATION_SHA,
          ...
          interpretation='Native-track target recovery; transition diagnostic supplies no accepted constraints'))
