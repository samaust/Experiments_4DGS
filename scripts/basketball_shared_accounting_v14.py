...
def report(out,state,scale,P,inverse,bound_inverse=None):
    ...
    if state['label']=='returned':
        ...
        if bound_inverse is not None:
            vnew=P.T@Cb;result.update(remapped_multiplier=vnew,remap_back=inverse.T@vnew,remap_role='new-P algebra only; original actual vby and saved bound transform retained')
        for frame in ('x','q','y'):
...
class Adapter:
    def __init__(self,namespace,state,scale,P,origin,inverse,provider,audit,bound_inverse=None):
        self.input=dict(state);self.scale=frozen_array(scale);self.P=frozen_array(P);self.origin=frozen_array(origin);self.inverse=frozen_array(inverse);self.provider=provider;self.audit=audit
        qb=bytes.fromhex(state['q_hex']);xb=bytes.fromhex(state['x_hex']);self.ledger=Ledger(namespace,{(qb,xb)})
        self.state=self.ledger.request(np.frombuffer(qb,dtype='<f8'),np.frombuffer(xb,dtype='<f8'));self.values=None;self.report_value=None;self.probe=None;self.requests=[]
        self.vg=state['vg_hex'];self.vby=state['vby_hex'];self.bound_inverse=bound_inverse
    ...
    def request(self,name,public=False,y=None,vg_hex=None,vby_hex=None):
        ...
        if self.values is None:self.values=freeze(self.ledger.enter(self.state,'complete',lambda:self.provider(self.input,self.audit)))
        if name=='R' and self.report_value is None:
            self.report_value=freeze(self.audit('report',lambda:report(self.values,self.input,self.scale,self.P,self.inverse,self.bound_inverse)))
    ...
    def fun(self,y):return self.request('O',True,y)
    def jac(self,y):return self.request('R',True,y)['frames']['y']['G']
    def hess(self,y):return self.request('R',True,y)['frames']['y']['H']
    def constraint(self,y):
        r=self.request('R',True,y);return r['g'],r['frames']['y']['Jg']
    def constraint_hessian(self,y,vg):
        return self.request('R',True,y,vg_hex=bytes_of(vg).hex())['frames']['y']['Hg']
