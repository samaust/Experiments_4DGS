#!/usr/bin/env python3
"""Plan023 immutable admission, one owned pass, and retained-only packaging."""
import argparse
import ast
import gzip
import hashlib
...
ROOT=Path('docs/experiments/basketball-shared-timing-v16')
AUTH=Path('docs/continuous-improvement/20260908-plan016/iteration-007-plan023-authorization.json')
SCRIPT=Path('scripts/basketball_shared_evaluator_v16.py')
TEST=Path('tests/test_basketball_shared_evaluator_v16.py')
...
SOURCES=[Path('scripts')/(s+'_v16.py') for s in ('basketball_acceleration_decimal','basketball_shared_components','basketball_shared_accounting','basketball_shared_solver','basketball_shared_reference','basketball_shared_evaluator')]
...
def admit():
 deadline('admission')
 previous=Path('docs/experiments/basketball-shared-timing-v15')
 import runpy
 predecessor=runpy.run_path('scripts/basketball_shared_evaluator_v15.py',run_name='predecessor_admission')
 predecessor['verify_chain']('worker')
 assert read(previous/'package-validation.json')['passed']
 assert sha(previous/'admission.json')=='0b64d71529fb0ac06e9a48998e53c3957b2d76249f64e3b1c48d96eaf32d1b18'
 ...
 for name in ('admission-chain.json','frozen.json','execution-sources.json','readiness.json','environment.json','historical-sources.json','package-validation.json','retention-manifest.json','decision.json','budget.json'):
  pinned[str(previous/name)]=sha(previous/name)
 for mapping in (read(previous/'admission-chain.json')['leaves'],read(previous/'historical-sources.json')['pinned'],read(previous/'frozen.json')['leaves'],read(previous/'execution-sources.json')['sha256']):
  hashes(mapping);pinned.update(mapping)
 ...
 complete_inputs=dict(cases=cases,synthetic_origin='v10 synthetic benchmark original observations; not reconstruction evidence')
 for side in ('candidate-inputs','reference-inputs'):
  assert hashlib.sha256(encode(complete_inputs)).hexdigest()==sha(previous/side/'inputs.json')
 selected=[cases[i] for i in (12,13)]
 assert [c['id'] for c in selected]==['metric/joint-00/descending','metric/joint-01/cold']
 assert [c['weight'] for c in selected]==[1,0]
 inputs={**complete_inputs,'cases':selected}
 for name in ('candidate-inputs','reference-inputs'):publish(ROOT/name/'inputs.json',inputs)
 selection=dict(original_indices=[12,13],full_input_hashes={side:sha(previous/side/'inputs.json') for side in ('candidate-inputs','reference-inputs')},selected=[dict(index=c['index'],id=c['id'],case_sha256=hashlib.sha256(encode(c)).hexdigest(),slots={v['label']:hashlib.sha256(encode(v)).hexdigest() for v in c['slots']}) for c in selected])
 publish(ROOT/'selection.json',selection)
 publish(ROOT/'archive.json',{str(i):archive[i] for i in (12,13)});publish(ROOT/'provenance.json',provenance)
...
def verify_chain(stage,root=ROOT,authorization=AUTH):
 ...
 if auth['phase_seconds']!=5400 or auth['scientific_invocations']!=1 or auth['toy_suite_invocations']!=2:raise ValueError('authorization limits')
 ...
 if root==ROOT:
  supplemental=root/'admission-package-supplement.json'
  if sha(supplemental)!='72403c81e0b375efac7da4cec3e47a306b3b5a6ac2260756a5a62cf46e926c05':raise ValueError('supplement mutation')
  supplement=read(supplemental);hashes(supplement['leaves'])
  if not supplement['passed'] or supplement['verified_monotonic']>auth['deadlines']['admission']['monotonic'] or supplement['original_admission_sha256']!=sha(root/'admission.json') or sha(supplement['retention_manifest_path'])!=supplement['retention_manifest_sha256']:raise ValueError('supplement chain')
...
def ready():
 ...
 try:
  ...
  leafs=[ROOT/'admission-chain.json',ROOT/'frozen.json',ROOT/'readiness-coverage.json',ROOT/'toy-definitions.json',ROOT/'coverage-source-review.md',ROOT/'comparison-schema.json',ROOT/'expected-schedule.json',ROOT/'arithmetic-contract.json',last]
...
def toy():
 ...
 os.environ['V16_TOY_TOKEN']=token;os.environ['V16_TOY_SLOT']=str(slot)
...
def toy_worker():
 slot=int(os.environ.get('V16_TOY_SLOT','0'));marker=ROOT/f'toy-suite-{slot:02}-consumed.json';started=read(marker)
 verify_chain('toy')
 worker_guard(started,os.environ.get('V16_TOY_TOKEN'),os.getppid(),ROOT/f'toy-suite-{slot:02}-worker.json')
 import runpy
 module=runpy.run_path(str(TEST),run_name='owned_toy_module')
 raise SystemExit(module['main'](slot,started,sha(marker)))


CAPS=dict(setup=2,complete=12,transform_context=2,prepare=14,objective_residual=12,residual_only=2,data_hessian=12,full_sum=12,raw_depth=12,bounded_values=12,weighted_raw_hessian=12,bounded_curvature=12,report=10,cold_report=2,svd_transform=2,saved_inverse=10,public_probe=10,extra_transport=2,acceleration=7)
REFCAPS={**CAPS,'complete':4,'prepare':4,'objective_residual':4,'data_hessian':4,'full_sum':4,'raw_depth':4,'bounded_values':4,'weighted_raw_hessian':4,'bounded_curvature':4,'report':2,'saved_inverse':2,'acceleration':2,'cold_report':2,'transform_context':0,'residual_only':0,'svd_transform':0,'public_probe':0,'extra_transport':0}
...
def transport_reference(reference,P,check=lambda:None):
 ...
 for k,v in reference['frames']['q'].items():
  ...
  if k in ('H','Hdata'):
   from basketball_shared_reference_v16 import symmetric_objective_congruence
   out['frames']['y'][k]=symmetric_objective_congruence(a,P,check)
  else:out['frames']['y'][k]=P.T@a@P if k.startswith('H') else a@P if k.startswith('J') else P.T@a
...
def worker():
 verify_chain('worker')
 started=read(ROOT/'started.json');worker_guard(started,os.environ.get('V16_TOKEN'),os.getppid(),ROOT/'worker-consumed.json')
 source_map=read(ROOT/'execution-sources.json')['sha256'];hashes(source_map);end=started['deadline_monotonic']-5
 import numpy as np
 import basketball_acceleration_decimal_v16 as decimal_candidate
 import basketball_shared_components_v16 as candidate
 import basketball_shared_accounting_v16 as accounting
 import basketball_shared_solver_v16 as solver
 import basketball_shared_reference_v16 as reference
 inner_counts={};active_audit=[None]
 def counted_boundary(module):
  def record(name):
   if time.monotonic()>=end:raise TimeoutError('inner numerical deadline')
   key=module+'.'+name;inner_counts[key]=inner_counts.get(key,0)+1
   if active_audit[0] is None or not active_audit[0].stack:raise ValueError('inner math without component ownership')
   owner=active_audit[0].stack[-1];owner.setdefault('inner_counts',{})[key]=owner.setdefault('inner_counts',{}).get(key,0)+1
  return record
 for module,name in ((candidate,'candidate'),(accounting,'accounting'),(reference,'reference')):module.boundary=counted_boundary(name)
 # Count unchanged function entries too; callbacks are checked before every call.
 def instrument(module,name,key):
  original=getattr(module,name)
  def wrapped(*a,**kw):counted_boundary(key)(name);return original(*a,**kw)
  setattr(module,name,wrapped)
 for name in ('basis','spatial'):instrument(reference,name,'reference_helpers')
 for name in ('projection_first','projection_second','robust_residual_jacobian'):instrument(candidate,name,'candidate_helpers')
 def save(path,value):
 ...
 cinputs=read(ROOT/'candidate-inputs/inputs.json');ca=Audit('candidate',end,source_map,ROOT/'entries.jsonl');setups={};active_audit[0]=ca
 for w in (0,1):
 ...
 for c in cinputs['cases']:
  ...
  ca.context_set(str(ci)+'/cold',cold);out=ca('complete',lambda:model.evaluate(cold,ca));cold_report=ca('cold_report',lambda:accounting.report(out,cold,scale,np.eye(m),np.eye(m),check=ca.check));save(f'candidate-{ci}-cold.json.gz',cold_report)
  for seq,order in enumerate(ORDERS,1):
   ...
   if seq==5:
    ...
    extra=returned;returned=ca('extra_transport',lambda:accounting.report(adapter.values,ret,scale,savedP,inverse,check=ca.check))
 ...
 rinputs=read(ROOT/'reference-inputs/inputs.json');ra=Audit('reference',end,source_map,ROOT/'entries.jsonl');rsetups={};active_audit[0]=ra
 for w in (0,1):
 ...
 for c in rinputs['cases']:
  ...
  for state in c['slots']:
   ...
   result=ra('cold_report' if label=='cold' else 'report',lambda:reference.report(out,state,scale,P,inverse,check=ra.check));save(f'reference-{c["index"]}-{label}.json.gz',result)
 ...
 da=Audit('driver',end,source_map,ROOT/'entries.jsonl',caps=dict(component_comparison=4,newP_comparison=2));summary=[];active_audit[0]=da;archive=read(ROOT/'archive.json')
 for c in rinputs['cases']:
  for label in ('cold','returned'):
   ...
   def component_context():
    ...
    if label=='returned':save(f'historical-{c["index"]}.json.gz',historical_comparisons(archive[str(c['index'])],ref,spec,da.check))
 ...
 save('comparison-summary.json',summary);save('inner-work-receipt.json',dict(counts=inner_counts))
 expected_inner=read(ROOT/'arithmetic-contract.json')['scientific_inner_counts']
 if inner_counts!=expected_inner:raise ValueError('scientific helper inventory mismatch')
 save('worker-complete.json',dict(monotonic=time.monotonic(),candidate_counts=ca.counts,reference_counts=ra.counts,driver_counts=da.counts,inner_counts=inner_counts))
...
def run():
 ...
 start=time.monotonic();auth=read(AUTH);end=min(start+600,auth['deadlines']['scientific_cleanup']['monotonic']);token=os.urandom(32).hex()
...
def package():
 ...
 admissible=all(decision['dimensions'][k]['complete'] and not decision['dimensions'][k]['failures'] for k in ('source_coverage','package_integrity','schedule'))
 decision.update(arithmetic_on_selected_four_states=dict(complete=decision['dimensions']['canonical']['complete'] and decision['dimensions']['transform']['complete'],passed=admissible and decision['dimensions']['canonical']['complete'] and decision['dimensions']['transform']['complete'] and not decision['dimensions']['canonical']['failures'] and not decision['dimensions']['transform']['failures'],original_indices=[12,13],coverage='4/32'),full_cohort_qualified=False,changed_source_unexecuted_states='unverified',legacy_replay_historical='v15: 16 sequence-5 rejections; preserved independently')
