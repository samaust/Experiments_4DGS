...
def verify_package(root,output):
    ...
    all_rows=[];details=[];unavailable=[]
    for folder in [root/'adapt',root/'benchmark']:
        for f in sorted(folder.glob('*/*-attempts.json.gz')):
            try:data=compressed_read(f)
            except (ValueError,EOFError,OSError) as error:
                unavailable.append(dict(path=str(f),sha256=sha256(f),error=type(error).__name__+': '+str(error)));continue
    ...
    for name in ['basketball','budget','selfcap','v9']:
        f=root/f'{name}-tests-final.log'
        if not f.exists():f=root/f'{name}-tests.log'
        s=f.read_text();assert re.search(r'\nOK\s*$',s),name
    ...
    write(output,dict(status='passed',stage_statuses=stages,attempt_records=len(all_rows),executed=sum(r.get('executed',True) for r in all_rows),qualified=sum(r['valid'] for r in all_rows),
        entry_verification=details,unavailable_attempt_artifacts=unavailable,scientific_qualification_verified=not bool(unavailable),tests=logs,partition=partition,markers=admission['markers'],inherited_hashes_verified=True,elapsed_seconds=time.time()-p['investigation_started_unix'],
