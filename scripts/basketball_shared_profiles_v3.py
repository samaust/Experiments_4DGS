...
from basketball_shared_solver_v3 import solve,provenance
...
def problem_key(group,cameras,edge,window,spacing,weight,role):
    return hashlib.sha256(json.dumps(dict(observations=group,calibration=cameras,edge=edge,
        window=window,spacing=spacing,weight=weight,role=role,solver=provenance()),
...
def attempt(group,cameras,edge,window,spacing,weight,lag,label,key,seed,check):
    ...
    try:
        ...
        fit=solve(problem,None if seed is None else seed[1]['x'])
...
def group_job(payload):
    ...
    if endpoint is not None:
        ...
        for lag in missing:
            ...
            row=dict(lag=lag,start='frozen_training',valid=bool(fit['valid'] and np.all(depth>0)),
...
def independent_edge(groups,cameras,edge,window,spacing,weight,check=lambda:None,
    ...
    try:
        for label,penalty in [('regularized',weight),('data_only',0.)]:
            states=[None]*len(selected);grid=np.arange(-25,26,dtype=float);fixed_ids=None
            for round_id in range(3):
                ...
                if round_id==0:fixed_ids=[s['group_id'] for s,good in zip(states,summary['support_mask']) if good]
...
def decision(motion,length,noise,truth,result,minimum_groups=12):
    ...
    else:passed=not result['passed'] or all(e is not None and e<=.25 for e in errors)
...
def safeguard(p,output):
    ...
    sources={f:sha256(f) for f in ['scripts/basketball_shared_profiles_v3.py','scripts/basketball_shared_solver_v3.py']}
