"""Plan 027 fixed comparisons and source-cadence videos for all four arms."""
import argparse
import json
...
def main():
    ...
    required = {(m, s) for m in arms for s in range(3)}
    if len(selected) != 12 or {(r['arm'], r['seed']) for r in selected} != required:
        raise ValueError('requires all twelve matched four-arm endpoint renders')
    ...
    def row(images, label):
        ...
        canvas = Image.new('RGB', (5*width, height+52))
        draw = ImageDraw.Draw(canvas)
        font = ImageFont.load_default(size=18 if width >= 480 else 11)
        draw.text((4, 3), label, fill='white', font=font)
        for i, (image, name) in enumerate(zip(images, ('Ground truth', 'STG Full', 'FreeTimeGS sparse', 'FreeTimeGS dense coarse', 'FreeTimeGS dense cropped'))):
    ...
    for seed in range(3):
        for camera, crops in protocol['cameras'].items():
            ...
            command = ['ffmpeg', '-hide_banner', '-loglevel', 'error', '-f', 'rawvideo',
                       '-pix_fmt', 'rgb24', '-s', '4800x592', '-framerate', '25', '-i', '-',
    ...
    write_new(a.output / 'artifacts.json', dict(schema='basketball-study-visuals/v1',
        ...
        artifacts=artifacts, arms=list(arms), complete=True))
