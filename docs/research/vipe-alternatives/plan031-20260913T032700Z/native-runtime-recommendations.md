...
metadata. For E1/E2/E4/E5/E7 use the official `cu124` index, E3 `cu128`, E6
...
| E4: D1, Python 3.11 | Install native UniDepth default requirements with `xformers==0.0.28.post3`, `torchaudio==2.5.1` from the matching cu124 stack and NumPy 2.1.3 constrained. Its requirements also include `einops>=0.7.0`, `gradio`, `h5py>=3.10.0`, `huggingface-hub>=0.22.0`, `imageio`, `matplotlib`, OpenCV, `pandas`, `pillow>=10.2.0`, `protobuf>=4.25.3`, `scipy`, `tables`, `tabulate`, `termcolor`, `timm`, `tqdm`, `trimesh`, `triton>=2.4.0`, `wandb`. The training-only KNN extension is not required by this inference adapter. |
...
```json
{
  ...
  "E4": {
    "requirements": [
      "xformers==0.0.28.post3", "torchaudio==2.5.1+cu124",
