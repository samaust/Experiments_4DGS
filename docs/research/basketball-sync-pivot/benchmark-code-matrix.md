...
| Sync-NeRF K-Planes | Released training implementation executes numerically on the RTX 4090 through the calibrated eight-camera adapter. Seed 0 reached 43,049 steps; fresh CPU reload matches the timing export. | Three-seed results are in the separately updated result artifact. Every pilot is capped below the 90,001-step author schedule. The native checkpoint lacks RNG/AMP-scaler state, so exact training continuation is not established. |
