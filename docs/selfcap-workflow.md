...
Validate hashes, camera membership/splits, timing/calibration and image sizes before training; the original SelfCap [preparation record](experiments/contender-data-20260906.md) describes those gates. New preparation can change metadata such as wall time and output paths, so do not substitute its manifest into an old checkpoint’s provenance. The [retained initialization record](experiments/007-freetimegs.md) documents historical EDGS-generated FreeTimeGS clouds and their training-only keyframe/successor exclusion gates. New clouds use `scripts/initialize-edgs-selfcap.py` with pinned RoMa and [local PyTorch triangulation](triangulation.md); keep the same frame/split gates. The script name remains compatible, but `--edgs` is optional, deprecated, ignored with a warning, and its path is never inspected. RoMa source and weight verification remains required.

New reports use `edgs-selfcap-cloud/v2` and record the local implementation hash, solver, float32 dtype, gels driver, rank threshold, and numerical rejection counts. Neighbors minimize `torch.cdist` over the same flattened camera transforms, excluding self; exact ties choose the lowest camera index in sorted training-key order. Positive depth and 0.01 normalized L1 reprojection gates remain. The dense loader accepts both version-1 and version-2 reports with the same input/archive hash and data validation. Existing archives, licenses, and experimental results retain their historical provenance; no production initializer was regenerated for this change.
