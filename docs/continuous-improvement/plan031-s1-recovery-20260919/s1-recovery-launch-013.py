...
def main():
    ...
    note = dict(schema="plan043-prospective-launch/v1", stage_start=START, observed=now,
        ...
        stdin_identity=dict(path=str(ROOT/"scripts/vipe_benchmark/s1_validation_contract.py"), sha256=hashlib.sha256((ROOT/"scripts/vipe_benchmark/s1_validation_contract.py").read_bytes()).hexdigest(), description="unchanged STDIN literal bound by source"), output_paths=[str(directory/"receipt.json"),str(directory/"execution.json")], new_scenarios_limit=24, new_scenario_execution_seconds=2, new_scenario_cleanup_seconds=1, environment=settings, unset_environment=["S1_HELPER_DIAGNOSTIC", "S1_RECEIPT_DIAGNOSTIC"], cpu_workers_max=8,
    ...
    result = dict(schema="plan043-driver-completion/v1", note=note_record, before_launch=before_launch,
