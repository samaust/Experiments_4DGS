#!/usr/bin/env python3
"""The frozen twenty Plan022 fixtures. Invocation marker precedes math imports."""
import ast
import json
import os
...
import basketball_shared_evaluator_v15 as harness


def main(slot,started,marker_hash):
    if os.environ.get('V15_TOY_TOKEN')!=started['token'] or not (harness.ROOT/f'toy-suite-{slot:02}-worker.json').exists():raise ValueError('direct test rejected before math import')
    ...
    import basketball_acceleration_decimal_v15 as dc
    import basketball_shared_components_v15 as cc
    import basketball_shared_accounting_v15 as ac
    import basketball_shared_reference_v15 as rr
        ...
        for w,t in retained.items():
            ...
            for seq,order in enumerate(harness.ORDERS,1):
                ...
                if seq==5:
                    ...
                    forbidden.update(('projection_second','spline_second','normalized_depth','objective_geometry'))
    ...
    def case19():
        ...
        for variant in range(1,11):
            ...
            result=harness.newp_comparisons(harness.plain(candidate),harness.plain(reference),transform,spec,0,check) if variant in (5,6,7) else harness.comparisons(harness.plain(candidate),harness.plain(reference),spec,check,policy)
    ...
    expected_counts={'basketball_acceleration_decimal_v15.setup':15,'basketball_acceleration_decimal_v15.bundle':19,'basketball_acceleration_decimal_v15.mapping':61,'basketball_shared_reference_v15.setup':2,'basketball_shared_reference_v15.basis':82,'basketball_shared_reference_v15.spatial':38,'basketball_shared_components_v15.projection_second':31,'basketball_shared_components_v15.projection_first':4,'basketball_shared_components_v15.robust_residual_jacobian':34,'Candidate.evaluate':18,'Reference.evaluate':4,'boundary.prepare_full':14,'boundary.prepare_first':2,'boundary.svd':2,'boundary.spline_second':28,'boundary.normalized_depth':28,'boundary.objective_geometry':28}
    inventory_passed=all(counts.get(k,0)==v for k,v in expected_counts.items());finish=time.monotonic()
    ...
    harness.publish(harness.ROOT/f'toy-suite-{slot:02}-protocol.json.gz',retained.get('protocol',{}))
