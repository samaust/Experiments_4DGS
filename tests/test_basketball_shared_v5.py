...
from basketball_shared_solver_v5 import ConstrainedProblem,sanitize,support,solve,EvaluationLimit,POLICY
from basketball_shared_spline_v2 import SplineProblem
from basketball_shared_synthetic_v2 import synthetic
from basketball_shared_profiles_v5 import group_job,attempt,summaries,problem_key,decision,basin_grid
...
class V5Tests(unittest.TestCase):
    ...
    def test_strict_distinct_evaluation_limit_including_derivatives(self):
        ...
        with patch('basketball_shared_solver_v5.MAX_EVALUATIONS',2):
    ...
    def test_no_fourth_attempt_deadline_and_cache_isolation(self):
        ...
        with patch('basketball_shared_profiles_v5.attempt',side_effect=fake):
        ...
        with patch('basketball_shared_profiles_v5.solve') as solver:
    ...
    def test_constraint_boundary_and_failed_termination_rejected(self):
        ...
        with patch('basketball_shared_solver_v5.minimize',side_effect=fake):
            row=solve(p)
    ...
    def test_fresh_stage_and_deadline(self):
        import tempfile
        from types import SimpleNamespace
        from basketball_shared_workflow_v5 import main,bounded
        with tempfile.TemporaryDirectory() as folder:
            with self.assertRaises(FileExistsError):main(SimpleNamespace(output=Path(folder)))
            from basketball_shared_workflow_v5 import prepare
            from basketball_scale import read
            original=read
            def changed(path):
                value=original(path)
                ...
                return value
            with patch('basketball_shared_workflow_v5.read',side_effect=changed),patch('basketball_shared_workflow_v5.verify_hashes'):
        ...
        with patch('basketball_shared_workflow_v5.time.time',return_value=5400.):
    ...
    def test_all_deadline_boundaries(self):
        from basketball_shared_workflow_v5 import bounded
    ...
    def test_bounded_transform_limits_and_underflow(self):
        from basketball_shared_solver_v5 import transform,MARGIN
    ...
    def test_boundary_dependent_confidence_rejected(self):
        with patch('basketball_shared_solver_v5.QUALIFICATION_DEPTH',10.):
            row=solve(self.fixture(weight=1.,lag=0.))
