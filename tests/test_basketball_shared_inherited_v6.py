"""Inherited contracts exercised on v6; unit-test solves are not the scientific pilot."""
import sys
import unittest
...
from basketball_shared_solver_v6 import ConstrainedProblem,sanitize,support,solve,EvaluationLimit,POLICY
from basketball_shared_spline_v2 import SplineProblem
from basketball_shared_synthetic_v2 import synthetic
from basketball_shared_profiles_v6 import group_job,attempt,summaries,problem_key,decision,basin_grid


class V6InheritedTests(unittest.TestCase):
    ...
    def test_seed_replacement_activation_and_deterministic_blending(self):
        ...
        for gid in [9,11]:
            ...
            self.assertEqual(len(x),len(seed));self.assertGreater(min(ConstrainedProblem(p).raw_depth(x/ConstrainedProblem(p).scale)[0]),2e-8)
    ...
    def test_group2_camera_plane_crossing_and_infeasible_cold(self):
        ...
        self.assertGreater(min(a.raw_depth(np.asarray(row['initial_x'])/a.scale)[0]),0)
        self.assertLess(min(a.raw_depth(np.asarray(row['x'])/a.scale)[0]),0)
    ...
    def test_no_fourth_attempt_deadline_and_cache_isolation(self):
        ...
        with patch('basketball_shared_profiles_v6.attempt',side_effect=fake):
        ...
        with patch('basketball_shared_profiles_v6.solve') as solver:
    ...
    def test_constraint_boundary_and_failed_termination_rejected(self):
        ...
        with patch('basketball_shared_solver_v6.minimize',side_effect=fake):
            row=solve(p)
    ...
    def test_fresh_stage_and_deadline(self):
        import tempfile
        from types import SimpleNamespace
        from basketball_shared_workflow_v6 import main,bounded
        with tempfile.TemporaryDirectory() as folder:
            with self.assertRaises(FileExistsError):main(SimpleNamespace(output=Path(folder)))
            from basketball_shared_workflow_v6 import prepare
            from basketball_scale import read
            original=read
            def changed(path):
                value=original(path)
                ...
                return value
            with patch('basketball_shared_diagnose_v6.read',side_effect=changed),patch('basketball_shared_diagnose_v6.verify_hashes'):
                with self.assertRaises(AssertionError):prepare(read('configs/basketball-rev2/timing-shared-v6.json'),Path(folder))
        with patch('basketball_shared_workflow_v6.time.time',return_value=5400.):
            with self.assertRaises(TimeoutError):bounded(dict(investigation_started_unix=0.),early=True)

    def test_analytical_ray_limits_and_original_barriers(self):
        from basketball_shared_diagnose_v5 import ray_limit
        p=self.fixture(weight=0.,lag=-6.,gid=9);x=p.x0.copy();direction=np.zeros_like(x)
        direction[p.n_offsets+2::3]=1.
        limit=ray_limit(p,x,direction)
        self.assertTrue(limit['ray_eventually_depth_feasible']);self.assertTrue(limit['acceleration_invariant_exact'])
        r=p.evaluate(x+1e10*direction)[0]
        np.testing.assert_allclose(r@r,limit['original_objective_limit'],rtol=1e-8)
        direction[:]=0;direction[1+3*17]=1.
        zero=ray_limit(p,x,direction)
        self.assertTrue(zero['exactly_unsupported']);self.assertFalse(zero['observable'])
        direction[:]=0;direction[3::3]=-1.
        self.assertFalse(ray_limit(p,x,direction)['ray_eventually_depth_feasible'])

    def test_all_deadline_boundaries(self):
        from basketball_shared_workflow_v6 import bounded
        from basketball_shared_diagnose_v5 import check_deadline
        with patch('time.time',return_value=1800.):
            with self.assertRaises(TimeoutError):bounded(dict(investigation_started_unix=0),early=True)
            with self.assertRaises(TimeoutError):check_deadline(dict(investigation_started_unix=0))
        with patch('time.time',return_value=13200.):
        ...
        with patch('time.time',return_value=14400.):
            with self.assertRaises(TimeoutError):bounded(dict(investigation_started_unix=0),package=True)

    def test_bounded_transform_limits_and_underflow(self):
        from basketball_shared_solver_v6 import transform,MARGIN
        s=np.array([-1e308,-1e44,-1.,-1e-12,0.,1e-12,1.,1e44,1e308])
        g,gp,gpp,flags=transform(s)
        np.testing.assert_array_equal(g>=0,s>=0)
        self.assertTrue(np.isfinite(g).all());self.assertTrue(flags['derivative_underflow'])
        self.assertGreater(flags['saturation'],0)
        self.assertEqual(g[4],0);self.assertEqual(gp[4],1);self.assertEqual(gpp[4],0)
        self.assertGreaterEqual(-.1*np.log(g[-2]),0)
        self.assertLess(-.1*np.log(s[-2]),-10)
        p=self.fixture(weight=1.);x=p.x0.copy();_,co=p.unpack(x)
        before=p.accel@co[0];co[0,:,2]+=100
        np.testing.assert_allclose(p.accel@co[0],before,atol=1e-10)
        self.assertEqual(ConstrainedProblem(p).fun(x/ConstrainedProblem(p).scale),float(p.evaluate(x)[0]@p.evaluate(x)[0]))

    def test_full_coefficient_hessian_and_multiplier_conversion(self):
        p=self.fixture();a=ConstrainedProblem(p);q=p.x0/a.scale
        g,J=a.depth(q);z,D=a.raw_depth(q);v=-np.ones(len(g))
        from basketball_shared_solver_v6 import transform,MARGIN
        gp=transform(z-MARGIN)[1]
        np.testing.assert_allclose(J.T@v,D.T@(v*gp),atol=1e-12)
        H=a.depth(q,v).toarray()
        self.assertGreater(np.max(np.abs(H[p.n_offsets:,p.n_offsets:])),0.)
    ...
    def test_boundary_dependent_confidence_rejected(self):
        with patch('basketball_shared_solver_v6.QUALIFICATION_DEPTH',10.):
            row=solve(self.fixture(weight=1.,lag=0.))
